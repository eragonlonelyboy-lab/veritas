...
2. **A named self-audit loop.** Before returning, it asks itself "what still reads as AI here?", answers in bullets, and revises to kill them. Then it greps its own output for `—` and `–`: any hit means it is not done. No humanizer audits its own output. For prose that carries real weight, escalate the audit to a fresh pass that sees only the rewrite and the rules: a reviewer blind to the drafting catches the tells the author defends. Length discipline follows the same honesty: shorten by dropping what the reader does not need, never by compressing what they do into fragments they must decode twice.
